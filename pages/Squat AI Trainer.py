...
if mode == "Upload video":
    ...
    if uploaded is None:
        ...
        output_path = os.path.join(tempfile.gettempdir(), "processed_squats.mp4")
    ...
    if os.path.exists(output_live):
        with open(output_live, "rb") as f:
            if st.download_button("📥 Download live recording", f, file_name="live_squats.flv"):
...
from utils import get_mediapipe_pose
from process_frame_squats import ProcessFrame
from thresholds_squats import get_thresholds_beginner, get_thresholds_pro
